...
router.get('/create-student', function(req,res){
  res.render('student-form.html')
})
...
router.post('/api/create', function(req, res){
    ...
    student.save(function(err,data){
      ...
      if (err){
        var error = {status:'ERROR', message: 'Error saving student'};
...
router.get('/api/get/:id', function(req, res){
  ...
  Student.findById(requestedId, function(err,data){
    ...
    if(err || data == null){
      var error = {status:'ERROR', message: 'Could not find that student'};
       return res.json(error);
    }
...
router.get('/api/get', function(req, res){
  ...
  Student.find(function(err, data){
    ...
    if(err || data == null){
      var error = {status:'ERROR', message: 'Could not find students'};
      return res.json(error);
    }
...
router.get('/api/search', function(req,res){
  ...
  Students.find({name: searchTerm}, function(err,data){
...
router.post('/api/update/:id', function(req, res){
   ...
   var dataToUpdate = {}; // a blank object of data to update
    ...
    if(req.body.id) {
      ...
      dataToUpdate['description']['id'] = id;
    }
    if(req.body.color) {
    ...
    Student.findByIdAndUpdate(requestedId, dataToUpdate, function(err,data){
      // if err saving, respond back with error
      if (err){
        var error = {status:'ERROR', message: 'Error updating student'};
        return res.json(error);
      }
...
router.get('/api/delete/:id', function(req, res){
  ...
  Student.findByIdAndRemove(requestedId,function(err, data){
    if(err || data == null){
      var error = {status:'ERROR', message: 'Could not find that student to delete'};
      return res.json(error);
    }
